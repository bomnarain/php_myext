...
dnl PHP_ARG_WITH([mytext],
dnl   [for mytext support],
dnl   [AS_HELP_STRING([--with-mytext],
dnl     [Include mytext support])])

dnl Otherwise use 'enable':

PHP_ARG_ENABLE([mytext],
  [whether to enable mytext support],
  [AS_HELP_STRING([--enable-mytext],
    [Enable mytext support])],
  [no])
...
if test "$PHP_MYTEXT" != "no"; then
  ...
  dnl PHP_EVAL_LIBLINE($LIBFOO_LIBS, MYTEXT_SHARED_LIBADD)
  ...
  dnl [
  dnl   AC_DEFINE(HAVE_MYTEXT_FEATURE, 1, [ ])
  dnl ],[
  dnl   AC_MSG_ERROR([FEATURE not supported by your mytext library.])
  dnl ], [
  dnl   $LIBFOO_LIBS
  dnl ])
  ...
  AC_DEFINE(HAVE_MYTEXT, 1, [ Have mytext support ])

  PHP_NEW_EXTENSION(mytext, mytext.c, $ext_shared)
fi
